...
const CONTROL =
  "w-full rounded-md border border-line bg-surface-2 px-2.5 text-ink " +
...
export function Input({
  ...
  return <input className={cn(CONTROL, "h-7", className)} {...rest} />;
}
...
export function Textarea({
  ...
  return (
    <textarea className={cn(CONTROL, "resize-y py-1.5", className)} {...rest} />
  );
}
...
export function Select({
  ...
  return (
    <select
      className={cn(
        CONTROL,
        "h-7 cursor-pointer appearance-none bg-no-repeat pr-7",
